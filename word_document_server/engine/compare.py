...
changed except this paragraph" and have that claim verified.  It is also what
the ``doc_compare`` V2 tool (:mod:`word_document_server.tools.v2.compare`)
builds on: the tool translates the snapshot-space paragraph keys read here
into the V2 locator vocabulary (:func:`~word_document_server.engine.find._v2_index_map`)
before reporting them to a caller, but the structural comparison itself --
what changed, not how to name it -- is entirely this module's.
