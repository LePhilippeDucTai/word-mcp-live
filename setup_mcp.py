...
def generate_mcp_config_uvx(transport_config):
    ...
    config = {
        "mcpServers": {
            "word-document-server": {
                "command": "uvx",
                "args": ["--from", "word-mcp-semantic", "word_mcp_server"],
                "env": env
            }
        }
    }
...
def install_from_pypi():
    ...
    try:
        subprocess.run([sys.executable, "-m", "pip", "install", "word-mcp-semantic"], check=True)
        print("word-mcp-semantic successfully installed from PyPI!")
        return True
    except subprocess.CalledProcessError:
        print("Failed to install word-mcp-semantic from PyPI.")
        return False
