...
- **`doc_compare`** — a new `doc_*` tool: structurally compares two documents and reports added, removed and changed paragraphs and tables, section changes and counter deltas (comments, revisions, bookmarks, fields, hyperlinks, footnotes, endnotes), so a caller can confirm that a batch of edits touched exactly what it meant to and nothing else.
