...
Lecture (`core/comments.py`) : ancres retrouvées (story, index V2, texte ancré) ; suppression du repli par `str(element)` ; erreurs remontées, plus d'`except Exception` silencieux.
...
### Scope
`format_text(paragraph_index, start_pos, end_pos, ...)` → `ranges.resolve` + `format.apply_rpr` ; offsets sur le texte visible du moteur ; index de paragraphe = espace V2 ; couleur invalide → message d'erreur, jamais noir ; plus aucun `run.clear()` ni `add_run`.
